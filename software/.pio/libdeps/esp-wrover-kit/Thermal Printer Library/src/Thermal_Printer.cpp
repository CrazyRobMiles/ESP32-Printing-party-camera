...
const char *szBLENames[] = {(char *)"MPT-II", (char *)"MTP-3", (char *)"MTP-3F", (char *)"GT01", (char *)"GT02", (char *)"GB01", (char *)"GB02", (char *)"YHK-A133", (char *)"PeriPage+", (char *)"PeriPage_", NULL};
      ...
      while (szMsg[i] && x < iPrintWidth)
      {
         ...
         for (ty = dy; ty <= end_y; ty++)
         {
            for (tx = 0; tx < pGlyph->width; tx++)
            {
               ...
               if (uc & 0x80 && ty == y)
               { // set pixel if we're drawing this line
                  ucMask = 0x80 >> ((dx + tx) & 7);
...
int tpDrawText(int x, int y, char *szMsg, int iFontSize, int bInvert)
{
   ...
   {
      iCursorX = x;
      iCursorY = y; // set the new cursor position
   }
...
static uint8_t tpFindPrinterName(char *szName)
{
   ...
   {
      if (strcmp(szName, szBLENames[i]) == 0)
      { // found a supported printer
...
int tpScan(const char *szName, int iSeconds)
{
   ...
   while (!bFound && (millis() - ulTime) < (unsigned)iSeconds * 1000UL)
   {
      ...
      if (peripheral)
      {
         ...
         { // compare the name with our supported ones
            ucPrinterType = tpFindPrinterName((char *)peripheral.localName().c_str());
            if (ucPrinterType < PRINTER_COUNT)
            {
               BLE.stopScan();
...
void tpSetFont(int iFont, int iUnderline, int iDoubleWide, int iDoubleTall, int iEmphasized)
{
   ...
   if (iFont < FONT_12x24 || iFont > FONT_9x17)
      return;
...
void tp1DBarcode(int iType, int iHeight, char *szData, int iTextPos)
{
   ...
   if (!bConnected || szData == NULL)
      return;
   len = (uint8_t)strlen(szData);
   ucTemp[i++] = 0x1d;
   ucTemp[i++] = 0x48;
   ucTemp[i++] = (uint8_t)iTextPos;
   ucTemp[i++] = 0x1d;
   ucTemp[i++] = 0x68;
   ucTemp[i++] = (uint8_t)iHeight;
   ucTemp[i++] = 0x1d;
   ucTemp[i++] = 0x77;
   ucTemp[i++] = 2; // width multiplier
   ucTemp[i++] = 0x1d;
   ucTemp[i++] = 0x6b;
   ucTemp[i++] = (uint8_t)iType;
   ucTemp[i++] = len;
...
void tpFeed(int iLines)
{
   ...
   else if (ucPrinterType == PRINTER_MTP2 || ucPrinterType == PRINTER_MTP3)
   {
      ...
      for (i = 0; i < iLines; i++)
      {
         ...
         ucTemp[3] = '0';
         ucTemp[4] = 1;
         ucTemp[5] = 0; // width = 1 byte
         ucTemp[6] = 1;
         ucTemp[7] = 0; // height = 1 line
...
static void tpPreGraphics(int iWidth, int iHeight)
{
   ...
   else if (ucPrinterType == PRINTER_MTP2 || ucPrinterType == PRINTER_MTP3)
   {
      ...
      ucTemp[4] = (iWidth + 7) >> 3;
      ucTemp[5] = 0;
      ucTemp[6] = (uint8_t)iHeight;
      ucTemp[7] = (uint8_t)(iHeight >> 8);
   ...
   else if (ucPrinterType == PRINTER_PERIPAGE || ucPrinterType == PRINTER_PERIPAGEPLUS)
   {
      ...
      ucTemp[1] = 0xff;
      ucTemp[2] = 0xfe;
      ucTemp[3] = 0x01; // start of command
      tpWriteData(ucTemp, 4);
      memset(ucTemp, 0, 12);
      ...
      ucTemp[2] = 0x30;
      ucTemp[3] = 0x00;
      ucTemp[4] = (uint8_t)((iWidth + 7) >> 3);
      ucTemp[5] = 0x00; // width in bytes
      ucTemp[6] = (uint8_t)iHeight;
      ucTemp[7] = (uint8_t)(iHeight >> 8); // height (little endian)
      tpWriteData(ucTemp, 8);
   }
